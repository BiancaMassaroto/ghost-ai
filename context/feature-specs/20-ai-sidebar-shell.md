Complete the existing AI sidebar placeholder and turn it into a proper floating chat sidebar component. The sidebar already exists, so keep the current floating placement and smooth slide-in behavior from the right side. This unit is focuser on building out the sidebar UI inside it.

## Implementation

1. Separate the AI sidebar into its own components.
...
4. Build the AI Architect tab.
   ...
   - user messages should be right-aligned with `bg-brand-dim border-brand/50 border-2 text-copy-prmary`
