Add shared AI activity indicators so everyone in the room can see when generation is in progress. This unit is only for UI, presence, and realtime status signals. Do not add actual AI generation flow yet.
...
3. Add status message validation.
   ...
   - the payload should support an opttional `text` field
