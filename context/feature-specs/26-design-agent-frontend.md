...
- On submit:
  ...
  - call `POST /api/ai/design` with `{prompt, projectId}` (no separate
    `roomId` field — the route derives the Liveblocks room id from the
    authorized `projectId` itself, since a client-supplied `roomId` can't be
    trusted to match; see the Architecture Decision in `progress-tracker.md`)
  - read `{runId}` from the response
  - call `POST /api/ai/design/token` with `{runId}` and read `{token}` from
    the response
- store `runId` and `token` in local state

2. Run status tracking

- Use `useRealtimeRun(runId, { accessToken: token })`
- While the run is active:
  - disable the chat input
  - show a loading state (spinner in the button is enough)
- When the run completes:
  - push a final AI message to `ai-chat` — a generic completion message on
    success (not the `ai-status-feed`'s "latest" message: it carries no run
    id, so it could belong to a different, concurrently-started run by the
    time this one settles), or the run's own error message on failure
  - reset loading + run state

3. Canvas updates (realtime)

- Do not menually update nodes/edges
